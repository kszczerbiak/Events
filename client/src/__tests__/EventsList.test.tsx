...
describe("<EventsList/>", () => {
  it("should render list of events", () => {
    ...
    const { getByText, getByTestId } = render(
      <EventsList connection={connection} events={list} />
    );
  ...
  it("should't render without connection to database", () => {
    const list = [
      {
        firstName: "Test",
        lastName: "Test",
    ...
    expect(eventList).not.toHaveTextContent(list[0].firstName);
    expect(eventList).not.toHaveTextContent(list[0].lastName);
    expect(eventList).not.toHaveTextContent(list[0].email);
    expect(eventList).not.toHaveTextContent(list[0].date);
    expect(eventList).not.toHaveTextContent(list[1].firstName);
    expect(eventList).not.toHaveTextContent(list[1].lastName);
    expect(eventList).not.toHaveTextContent(list[1].email);
    expect(eventList).not.toHaveTextContent(list[1].date);
  });
});
